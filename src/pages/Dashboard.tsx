...
@media (max-width: 639px) {
  .dash-hero-title { font-size: 1.75rem !important; }
  .dash-weather-grid { grid-template-columns: repeat(2, 1fr) !important; }
  .dash-stat-value { font-size: 1.5rem !important; }
  .dash-map-height { height: 260px !important; }
  .dash-rag-orb { width: 3.5rem !important; height: 3.5rem !important; }
}
`;
...
const Dashboard: React.FC = () => {
  ...
  // Scouting snapshots per tree for Growth Analytics + Tagged Trees
  ...
  useEffect(() => {
    ...
    treeTags.forEach(tag => {
      ...
      // Slightly larger marker with a subtle ring so trees are easy to tap
      ...
      marker.addListener('click', () => {
        // 1. Pan & zoom map to this tree
        map.panTo(pos);
        map.setZoom(19);
        setSelectedFieldId(tag.fieldId);
        setSelectedTreeId(tag.id);
        // 2. Show a brief infoWindow with tree name + health status (uses latest snapshots via ref)
        ...
        // 3. Open the full scouting details modal
  ...
  // Update tree marker icons to highlight selected tree with a gold ring
  useEffect(() => {
  ...
  const handleViewTree = (tag: { id: string; latitude: number; longitude: number; fieldId: string }) => {
    ...
    // Scroll the map into view so the user can see the tree highlighted
  ...
  // Keep refs in sync so map marker click handlers always use fresh state/functions
  ...
  return (
    <>
      ...
      <div className="dash-page-bg space-y-4 sm:space-y-6 lg:space-y-8 pb-12 px-0">
        ...
        <div className="dash-fade-down dash-d0 relative overflow-hidden rounded-3xl dash-header-banner shadow-2xl mx-0">
          ...
          <div className="relative px-4 sm:px-8 py-8 sm:py-10 flex flex-col items-center text-center gap-3 sm:gap-4">
            {/* Live badge */}
            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1 sm:py-1.5 bg-white/15 backdrop-blur-sm border border-white/30 rounded-full text-emerald-100 text-xs font-semibold tracking-wide">
              <span className="relative inline-block w-2 h-2 rounded-full bg-emerald-300 dash-pulse" />
              Season 2026 · Live
            </div>

            <h1 className="dash-hero-title text-3xl sm:text-4xl md:text-5xl font-black text-white drop-shadow-lg tracking-tight leading-tight">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {stats.map((s, i) => (
            <div
              key={i}
              onClick={s.onClick}
              className={`dash-scale-in ${s.delay} dash-stat-card rounded-2xl p-4 sm:p-5 flex flex-col items-center text-center ${s.onClick ? 'cursor-pointer' : ''}`}
            >
              <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br ${s.color} flex items-center justify-center text-xl sm:text-2xl shadow-md mb-2 sm:mb-3`}>
                {s.icon}
              </div>
              <p className="dash-stat-value text-2xl sm:text-3xl font-extrabold text-gray-900 tabular-nums">{s.value}</p>
              <span className="text-[10px] sm:text-xs font-semibold text-gray-400 uppercase tracking-widest mt-1">{s.title}</span>
              {s.onClick && (
        ...
        {/* ══════════════════════════════════════════════
            HEALTH RAG MATRIX
        ══════════════════════════════════════════════ */}
        <div className="dash-fade-up dash-d2 dash-glass-card rounded-2xl p-4 sm:p-6">
          <div className="flex flex-col items-center text-center mb-4 sm:mb-6 gap-1">
            ...
            {[
              ...
              return (
                <div key={label} className="flex flex-col items-center gap-2 flex-1">
                  <div
                    className={`dash-rag-orb w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center shadow-md ${rc.ring} ${rc.bg} relative`}
                    title={tooltip}
                  >
                    <Icon className={`w-6 h-6 sm:w-7 sm:h-7 ${rc.text}`} />
                  </div>
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {(selectedFieldId ? treeTags.filter(t => t.fieldId === selectedFieldId) : treeTags).map(tag => {
                  ...
                  return (
                    <div
                      key={tag.id}
                      className={`dash-field-card shrink-0 min-w-40 p-2.5 rounded-xl cursor-pointer transition-all ${
                        isSelected
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: meta ? meta.dot : getVarietyColor(tag.variety) }} />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-gray-900 truncate">{tag.name || 'Tree'}</p>
                          <p className="text-xs text-gray-400 truncate">{tag.variety || 'Unknown variety'}</p>
                        </div>
        ...
        {forecast.length > 0 && (
          <div className="dash-fade-up dash-d4 dash-glass-card rounded-2xl p-6">
            ...
            <div className="dash-weather-grid grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2 sm:gap-3 mb-6">
              {forecast.map((day, i) => {
          ...
          {smartActions.length === 0 ? (
            ...
            <div className="space-y-3">
              {smartActions.map((action, idx) => (
                <div
                  key={idx}
                  className={`dash-action-row dash-slide-r dash-d${idx + 1} flex flex-col sm:flex-row items-start gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl border`}
                  style={{
        ...
        {/* ══════════════════════════════════════════════
            PRODUCTION OVERVIEW + GROWTH ANALYTICS (SCOUTING)
        ══════════════════════════════════════════════ */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          {/* Production Overview — placeholder */}
          <div className="dash-fade-up dash-d5 dash-glass-card rounded-2xl p-6">
            <div className="flex flex-col items-center text-center mb-4 gap-2">
              <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-emerald-500 to-green-400 flex items-center justify-center text-xl shadow-md">
                📈
              </div>
              <h3 className="text-lg font-extrabold text-gray-900">Production Overview</h3>
            </div>
          ...
          {/* Growth Analytics — Tree Scouting Health Breakdown */}
          <div className="dash-fade-up dash-d6 dash-glass-card rounded-2xl p-6">
            <div className="flex flex-col items-center text-center mb-4 gap-2">
              <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-teal-500 to-emerald-400 flex items-center justify-center text-xl shadow-md">
                🌳
              </div>
              <h3 className="text-lg font-extrabold text-gray-900">Growth Analytics</h3>
              <p className="text-xs text-gray-400">Tree scouting health distribution</p>
            </div>

            {treeHealthSnapshots.length === 0 ? (
              <div className="h-44 bg-gray-50 rounded-xl flex flex-col items-center justify-center border border-dashed border-gray-200 gap-2">
                <span className="text-3xl">🌿</span>
                <p className="text-sm text-gray-400">No scouting data yet</p>
                <button
              ...
              return (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-around py-2 px-3 bg-gray-50 rounded-xl border border-gray-100">
                    {bars.map(b => (
                      <div key={b.label} className="flex flex-col items-center gap-0.5">
                        <span className="text-lg">{b.icon}</span>
                        <span className="text-base font-extrabold text-gray-900">{b.count}</span>
                        <span className="text-[9px] font-semibold text-gray-400 uppercase tracking-wide">{b.label}</span>
                      </div>
                    ))}
                  </div>
                    ...
                    {bars.map(b => {
                      ...
                      return (
                        <div key={b.label} className="flex items-center gap-2">
                          <span className={`text-[10px] font-bold w-14 shrink-0 ${b.textColor}`}>{b.label}</span>
                          <div className="flex-1 h-2.5 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              className={`h-2.5 rounded-full ${b.bg} transition-all duration-700`}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
      ...
      {scoutingModalTree && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-4 dash-header-banner">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-extrabold text-white">{scoutingModalTree.tag.name || 'Tree'}</h2>
                  <p className="text-xs text-emerald-200">{scoutingModalTree.tag.variety || 'Unknown variety'} · Scouting Details</p>
                </div>
              </div>
              <button
